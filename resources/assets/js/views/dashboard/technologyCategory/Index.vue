    ...
    <div>
        ...
        <vue-table apiUrl="technologyCategory" :tableClass="tableClass" :fields="fields" :itemActions="itemActions"
            ...
            <template slot="buttons">
                <router-link to="/dashboard/technologyCategory/create" class="btn btn-primary">创建</router-link>
            </template>
        </vue-table>
    </div>
</template>
...
//                    {name: 'view-item', icon: 'fa fa-eye', class: 'btn btn-success'},
        ...
        methods: {
            tableActions(action, data) {
                if (action == 'edit-item') {
                    this.$router.push('/dashboard/technologyCategory/' + data.id + '/edit');
                } else if (action == 'delete-item') {
                    this.$http.delete('technologyCategory/' + data.id)
                        .then((response) => {
                            toastr.success('删除成功！')
                            this.$emit('reload')
                        })
