<template>
    <div>
        ...
        <vue-table apiUrl="skillInfo" :tableClass="tableClass" :fields="fields" :itemActions="itemActions"
                   @table-action="tableActions" showPagination>
            <template slot="buttons">
                <router-link to="/dashboard/skillInfo/create" class="btn btn-primary">创建</router-link>
            </template>
        </vue-table>
    </div>
</template>
...
//                    {name: 'view-item', icon: 'fa fa-eye', class: 'btn btn-success btn-sm'},
        ...
        methods: {
            tableActions(action, data) {
                if (action == 'edit-item') {
                    this.$router.push('/dashboard/skillInfo/' + data.id + '/edit');
                } else if (action == 'delete-item') {
                    this.$http.delete('skillInfo/' + data.id)
                        .then((response) => {
                            toastr.success('删除成功！')
                            this.$emit('reload')
                        }).catch((error) => {
                    ...
                    this.$router.push({
                        path: `/dashboard/skillInfo/${data.id}/skill`,
                        query: {
                            pName: data.name
                        }
                    });
