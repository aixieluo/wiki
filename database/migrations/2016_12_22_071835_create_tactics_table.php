...
class CreateTacticsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('tactics', function (Blueprint $table) {
            ...
            $table->float('fire_down', 5, 2)->default(0)->comment('降低装备火力至');
            $table->float('penetrate_down', 5, 2)->default(0)->comment('降低装备穿甲至');
            $table->float('durable_down', 5, 2)->default(0)->comment('降低装备耐久至');
            $table->float('armor_down', 5, 2)->default(0)->comment('降低装备装甲至');
            $table->float('hit_down', 5, 2)->default(0)->comment('降低装备命中至');
            $table->float('dodge_down', 5, 2)->default(0)->comment('降低装备闪避至');
            $table->float('concealment_down', 5, 2)->default(0)->comment('降低装备隐蔽至');
            $table->float('spy_down', 5, 2)->default(0)->comment('降低装备侦查至');
